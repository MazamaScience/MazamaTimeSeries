test_that("days and weeks parameters work", {
  start_PDT <- min(example_sts$data$datetime)
  max_PDT <- max(example_sts$data$datetime)
  
  days_1 <- ISOdatetime(2018, 08, 02, 06, 59, 00, tz = "UTC")
  days_5 <- ISOdatetime(2018, 08, 06, 06, 59, 00, tz = "UTC")
  days_7 <- ISOdatetime(2018, 08, 08, 06, 59, 00, tz = "UTC")
  days_14 <- ISOdatetime(2018, 08, 15, 06, 59, 00, tz = "UTC")
  
  expect_identical(
    c(start_PDT, days_1),
    range( sts_filterDate(example_sts, 20180801, days = 1)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, days_5),
    range( sts_filterDate(example_sts, 20180801, days = 5)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, days_7),
    range( sts_filterDate(example_sts, 20180801, days = 7)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, days_14),
    range( sts_filterDate(example_sts, 20180801, days = 14)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, days_7),
    range( sts_filterDate(example_sts, 20180801, weeks = 1)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, days_14),
    range( sts_filterDate(example_sts, 20180801, weeks = 2)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, max_PDT),
    range( sts_filterDate(example_sts, 20180801, days = 100000000)$data$datetime )
  )
  
  expect_identical(
    c(start_PDT, max_PDT),
    range( sts_filterDate(example_sts, 20180801, weeks = 100000000)$data$datetime )
  )
  
  # expect errors for non-numeric inputs
  
  expect_error(
    sts_filterDate(example_sts, 20180801, days = "1")
  )
  
  expect_error(
    sts_filterDate(example_sts, 20180801, weeks = "1")
  )
...
test_that("character date formats work with timezones", {
  ...
  expect_identical(
    c(start_HST, end_HST),
    range( sts_filterDate(example_sts, 20180802, 20180803, timezone = "US/Hawaii")$data$datetime )
  )
...
test_that("POSIXct date formats work with timezones", {
  ...
  expect_identical(
    c(start_HST, end_HST),
    range( sts_filterDate(example_sts, start_Hawaii, end_Hawaii, timezone = "US/Hawaii")$data$datetime )
  )
